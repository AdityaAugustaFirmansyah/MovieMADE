...
class MovieFavouriteAdapter(private val movies:List<com.aditya.core.domain.model.Movie>) : RecyclerView.Adapter<MovieFavouriteAdapter.Holder>() {
    ...
    class Holder(private val binding:ItemMovieBinding):RecyclerView.ViewHolder(binding.root) {
        ...
        fun bind(movieEntity: com.aditya.core.domain.model.Movie) {
            binding.apply {
                Glide.with(itemView).load(com.aditya.core.data.source.remote.network.ApiClient.BASE_URL_IMAGE+movieEntity.posterPath).error(R.drawable.ic_broken_image_black).into(poster)
